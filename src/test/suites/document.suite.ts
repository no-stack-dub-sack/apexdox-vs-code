...
export const createDocumentSuite = (files: ITestFile[]) => {
  suite('Document', () => {
    ...
    test('Should place classes in correct groups according to "@group" tags', function () {
      ...
      const assertionGroup = $('a', '#Assertions')
        .toArray()
        .map((el) => $(el).text());
      const featureGroup = $('a', '#Feature1')
        .toArray()
        .map((el) => $(el).text());
      const miscellaneousGroup = $('a', '#Miscellaneous')
        .toArray()
        .map((el) => $(el).text());
    ...
    test('Should have a menu item for each file', function () {
      ...
      const offendingMenuItems = $('li.nav-item')
        .toArray()
        .map((el) => $(el).text().trim())
    ...
    test('Should capture tag values over multiple lines', function () {
      ...
      const param = $('.param-description')
        .toArray()
        .filter((el) => $(el).text().trim() === 'Works over multiple lines.');
      assert.equal(
        $(param).text().trim(),
        'Works over multiple lines.',
        'param description value does not match expected multi-line value.'
      );
      ...
      assert.equal(
        ...
        'Works over multiple lines.',
        'description value does not match expected multi-line value'
      );

      const author = $('.method-subtitle-description')
        .toArray()
        .filter((el) => $(el).text().trim() === 'Works over multiple lines (author).');
      assert.equal($(author).text().trim(), 'Works over multiple lines (author).', 'author value does not match expected multi-line value');

      const since = $('.method-subtitle-description')
        .toArray()
        .filter((el) => $(el).text().trim() === 'Works over multiple lines (since).');
      assert.equal($(since).text().trim(), 'Works over multiple lines (since).', 'since value does not match expected multi-line value');

      const returns = $('.method-subtitle-description')
        .toArray()
        .filter((el) => $(el).text().trim() === 'Works over multiple lines (returns).');
      assert.equal(
        $(returns).text().trim(),
        'Works over multiple lines (returns).',
        'returns value does not match expected multi-line value'
      );

      const exception = $('.method-subtitle-description')
        .toArray()
        .filter((el) => $(el).text().trim() === 'Works over multiple lines (exception).');
      assert.equal(
        $(exception).text().trim(),
        'Works over multiple lines (exception).',
        'exception value does not match expected multi-line value'
      );

      const deprecated = $('.method-subtitle-description')
        .toArray()
        .filter((el) => $(el).text().trim() === 'Works over multiple lines (deprecated).');
      assert.equal(
        $(deprecated).text().trim(),
        'Works over multiple lines (deprecated).',
        'deprecated value does not match expected multi-line value'
      );
    });
