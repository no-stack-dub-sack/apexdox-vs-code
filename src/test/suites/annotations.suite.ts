...
export const createAnnotationsSuite = (files: ITestFile[]) => {
  suite('Annotations', function () {
    ...
    test('Should correctly capture annotations with params', function () {
      ...
      const annotations2 = last(
        methods.filter((el) => $(el).text().trim() === 'public static void method3()').map((el) => $(el).prev().text())
      );
      const annotations3 = last(
        methods.filter((el) => $(el).text().trim() === 'public static void method4()').map((el) => $(el).prev().text())
      );
