...
class ApexDoxError extends Error {
    public static INVALID_SCOPE =
        'Please provide an array of valid scopes. Valid scopes include: global, public, protected, private, testMethod, and webService';
    ...
    public static INVALID_PORT = (port: number) => `Value for 'port' parameter '${port}' is invalid. Please provide a valid port number.`;
    ...
    public static INVALID_TYPE = (arg: string, type: string) => `Value for '${arg}' parameter is incorrect type. Expected '${type}'`;
    ...
    public static UNKNOWN_CONFIG_SETTING = (field: string) =>
        `Unknown configuration setting '${field}' found. Please check your settings file.`;
}
