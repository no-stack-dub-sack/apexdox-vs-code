...
class Validator {
    ...
    public validate(): IApexDocConfig {
        const keys = Object.keys(this.config);
        for (var key of keys) {
            // we validate port setting only when running doc server.
            // also prevent errant fields included in .apexdoc2rc file
            // from calling validator method and throwing an error here.
    ...
    // #region Field Validators / Instance Methods
    private includes() {
        Validator.stringArray(this.config.includes, 'includes');
    }

    private excludes() {
        Validator.stringArray(this.config.includes, 'excludes');
    }

    private cleanDir() {
        this.config.cleanDir = Validator.boolGuard(this.config.cleanDir, false);
    }

    private showTOCSnippets() {
        this.config.showTOCSnippets = Validator.boolGuard(this.config.showTOCSnippets, true);
    }

    private subtitle() {
        Validator.typeGuard('string', this.config.subtitle, 'subtitle');
    }

    private pages() {
        this.config.pages = Validator.stringArray(this.config.pages, 'pages')
            .map(path => Validator.directory(path, 'pages', '.html'));
    }

    private assets() {
        // do not validate directory on assets, user will be warned at runtime
        this.config.assets = Validator.stringArray(this.config.assets, 'assets')
            .map(path => Utils.resolveWorkspaceFolder(path));
    }

    private targetDirectory() {
        const path = this.config.targetDirectory;
        Validator.typeGuard('string', path, 'targetDirectory');
    ...
    private homePagePath() {
        Validator.typeGuard('string', this.config.homePagePath, 'homePagePath');
        this.config.homePagePath = Validator.directory(this.config.homePagePath, 'homePagePath', '.html');
    }

    private sortOrder() {
        let sortOrder = this.config.sortOrder;
        Validator.typeGuard('string', sortOrder, 'sortOrder');
    ...
    private title() {
        Validator.typeGuard('string', this.config.title, 'title');
        this.config.title = this.config.title || 'Apex Documentation';
    }

    private scope() {
        const scopes = this.config.scope;
        Validator.typeGuard('array', scopes, 'scope');
    ...
    private source() {
        this.config.source = this.config.source.map(src => ({
            path: Validator.directory(src.path, 'source.path'),
            sourceUrl: Validator.sourceUrl(src.sourceUrl)
        }));
    }
    // #endregion
    ...
    private static boolGuard(bool: boolean, defaultValue: boolean): boolean {
